...
public class Main
    ...
    public CommunicationManager communication = null, communication_kitchen = null;
        ...
        if (bright) {
            ...
            if (elapsed > duration) {
                ...
                if (overlayer != null) {
                    ...
                    long id = elapsed / duration;
                    if (id != lastID && settings.cur_device != null) {
                        ads.setVisibility(View.INVISIBLE);
                        ads.loadUrl("http://"+settings.cur_device.IP+":9900/ads?id=" + Long.toString(id));
                        lastID = id;
                    }
                }
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ads.setVisibility(View.INVISIBLE);
        WebSettings websettings = ads.getSettings();
        ...
        ads.setWebChromeClient(new WebChromeClient() {
            @Override
            public void onReceivedTitle(WebView view,
                                        String title) {
                if (title.contains("صفحة الويب غير متوفرة") || title.contains("Webpage not available")) {
                    ads.setVisibility(View.INVISIBLE);
                } else {
                    ads.setVisibility(View.VISIBLE);
                }
...
//        communication_kitchen = CommunicationManager.Create("Kitchen Writer", new CommunicationManager.ServerDataCallback() {
//            @Override
//            public void onServerData(final int[] arg1, final int[] arg2, final int[] arg3, final int[] arg4, final int[] arg5) {
//                runOnUiThread(new Runnable() {
//                    @Override
//                    public void run() {
//                        rooms.get("Kitchen").onServerData(arg1, arg2);
//                    }
//                });
//            }
//        });
        ...
        if(parent.getAdapter() == room_adapter) {
            ...
            if (str.equals("Settings")) {
                ...
                if (cur_room_feature != rooms.get("Kitchen")) {
                    ...
                    SetCurrentFeature(rooms.get("Kitchen"));
                    communication_kitchen.SetServerIP(new_device.IP);
                }
            }
        }
    }
            ...
            public void onDeviceFound(final String addr, final String text, final int type, final String data) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (type == 6) { // kitchen
                            d.IP = "k" + d.IP; // k<IP> instead of <IP>
